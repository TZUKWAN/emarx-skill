...
EMARX 是一个面向 Codex 的中文学理思辨论文生产系统。v6.6 版本的重点不再只是“把论文写得更像论文”，而是建立一套完整流程：本地资料优先读取、逐篇文章拆解、三至五篇锚定、内部影子重组、逻辑骨架生成、段落级详细大纲、逐段写作、结构防火墙、大家风范语体校准、匿名评审式审稿、改稿深化、事实核查、Word 交付，以及用户研究画像迭代。
...
- 本地资料优先，不再凭空写作
- 可基于工作空间全文抽取结果，为每篇文章生成拆解报告
- 可根据用户题目从本地拆解报告中锚定 3-5 篇论文
...
EMARX v6.6 继承并尊重此前全文蒸馏形成的写作规律。该批材料的已验证记录为：
...
`references/distillation-evidence.md` 中保留了标题、摘要、引言、正文结构、段落模板、文风控制、审稿规则等从真实文献中提炼出的规律。v6.6 的新流程是在这些规律之上搭建，而不是替代它们。
...
因此，EMARX 把“10,000-12,000 字正文”和“正文主体必须有二级标题”写成硬规则。v6.5 进一步要求标题层级必须是论文的论证骨架，而不是内部研究流程。v6.6 增加全量文章拆解、三至五篇锚定、影子重组和逐段写作流程。此前没有二级标题、篇幅过短、把诊断项写成章节标题、或者一次性生成多段后再泛泛润色的输出，均视为不合格。

## 大家风范语体

EMARX v6.6 的目标语言不是晦涩的“学术腔”，也不是普通评论式白话，而是：

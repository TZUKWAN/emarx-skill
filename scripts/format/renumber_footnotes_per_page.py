...
def get_footnote_pages_via_word(docx_path):
    """用 Word/WPS COM 打开文档，返回 [页码] 列表，按脚注顺序（1起）。

    静默原则：
    - 优先复用用户已打开的 Word/WPS 实例（GetActiveObject），避免新建窗口抢占前台。
    - 没有已运行实例时才新建隐藏实例（Dispatch）。
    - 仅当本脚本新建了实例时才 Quit；复用的实例保持原状，不影响用户前台工作。
    """
    ...
    # 1. 尝试连接已运行的 Word/WPS 实例（多种 ProgID）
    ...
    if word is None:
        raise RuntimeError("无法连接或创建 Word/WPS COM 实例，请确认已安装 Microsoft Word 或 WPS Office")

    try:
        # 3. 打开文档、查页码、关闭文档（ReadOnly + 不保存）
        ...
        # 4. 只有本脚本新建的实例才退出
        if created_by_us and word is not None:
            try:
                word.Quit()
            except Exception:
                pass

...
1. 当用户要求使用 CNKI 检索、Word 交付或任何 EMARX 脚本时，AI 直接调用目标脚本；
2. 每个脚本开头会自动调用 `emarx_env.ensure_venv_and_reexec()`：
   - 若 EMARX 专用虚拟环境（`.venv/`）不存在，自动创建；
   - 若虚拟环境中缺少依赖，自动补装；
   - 若缺少 Playwright Chromium 浏览器，自动下载；
   - 一切就绪后，自动切换到虚拟环境继续运行原命令；
3. 首次配置可能需要几分钟（下载浏览器和依赖），AI 应告知用户正在自动配置；
4. 若自动安装失败，AI 应明确告知用户失败原因和手动修复命令。

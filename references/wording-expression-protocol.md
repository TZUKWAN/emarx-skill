...
这是段落缺乏推进力时的修复工具，不是每段都必须遵守的固定模板。不同段落可根据任务选择不同进入方式（见 `generative-writing-protocol.md` 中的“不同段落的不同进入方式”）。
...
元话语是句子在解释论文自身而非论述对象。它让 EMARX 听起来像助手在解释作业，或在正文里插入了一段综述。正文、摘要、引言、结论中应直接不出现。
...
- **审稿回应信、修改说明、交付说明**：这些是与编辑/读者沟通的元文本，不是论文论证本身；
- **脚注中的自我说明**：如"参见本文第 X 页"，但应尽量少用；
- **学位论文的章节过渡段**：可适当使用，但仍以对象和逻辑承接为主。

### 改写方法

遇到元话语时，按以下方向改写：

1. **删除主语**：把"本文认为 X"改为直接陈述"X"；
2. **消化文献**：把"有研究指出 X[1]"改为"X[1]"，让文献编号紧跟被支撑的判断；
3. **对象前置**：把"前文已经说明 X，本文进一步分析 Y"改为"X 改变了 Y 的前提，因此需要回到……"；
4. **删除结构预告**：把"本文将从 A、B、C 三个方面分析"直接删除，由标题层级体现结构。

### 正例与反例

**反例**：

> 本文认为，生成式人工智能对中华文化国际传播有重要影响。有研究指出，人工智能可以提升翻译效率[1]。因此，本文将从传播机制、风险生成和治理路径三个方面进行分析。

问题：作者自我说明、无内容引语、结构预告，三处元话语叠加。

**正例**：

> 生成式人工智能进入国际传播后，文化内容的生产、调用和跨语境适配方式发生了深层变化。模型训练数据、平台分发逻辑与用户提示共同塑造新的意义生成环境，扩大了中华文化的多语言、多模态扩散能力，也增加了文化语义被稀释的风险[1]。

改进：对象直接呈现，文献编号紧跟被支撑的判断，没有元话语。
